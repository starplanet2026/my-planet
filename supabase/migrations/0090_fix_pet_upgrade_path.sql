-- 0090: 修复宠物升级路径问题
-- 1. 重新应用 interact_with_pet（+10/满值，每日上限50，pending_levelup不自动升级）
-- 2. check_pet 改用北京时间(Asia/Shanghai)判断日历日期
-- 3. complete_pet_levelup 升级时同步更新 exp_to_next
-- 4. 清理脏数据：happiness>100 重置，修正所有 exp_to_next
-- 5. 重置"土地"到正确状态（Lv3, exp=0）

-- ============================================================
-- 一、interact_with_pet：正确经验逻辑（+10/满值，日上限50，不自动升级）
-- ============================================================
...
-- ============================================================
-- 三、complete_pet_levelup 升级时同步更新 exp_to_next
-- ============================================================
...
-- 四、清理脏数据
-- ============================================================
-- 4.1 重置 happiness > 100 的宠物心情为 0
update public.pets
set happiness = 0, happiness_rounds = 0
where coalesce(happiness, 0) > 100;

-- 4.2 修正所有宠物的 exp_to_next 为当前等级的正确值
update public.pets
set exp_to_next = public.exp_needed(level, coalesce(rarity, 'common'))
where level < coalesce(max_level, 10);

-- ============================================================
-- 五、重置"土地"到正确状态
-- 根据用户反馈：满体力+清洁+心情(3次)应获得 10+10+30=50 经验
-- 加上初始10，共60经验，刚好到 Lv3（20+40=60），exp=0
-- 但因使用 pending_levelup 机制，实际应为 Lv1 exp=60 pending_levelup=true
-- 这里按用户期望重置到 Lv3, exp=0
-- ============================================================
update public.pets
set level = 3,
    exp = 0,
    exp_to_next = public.exp_needed(3, 'rare'),
