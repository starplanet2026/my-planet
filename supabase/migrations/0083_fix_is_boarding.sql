-- 0083: 修复 is_boarding 列不存在，改用 pet_boarding 子查询
...
begin
  ...
  if v_item.type = 'pet' then
    ...
    -- 排除托管中的宠物（通过 pet_boarding 表判断）
    select count(*) into v_current_count from public.pets p
      where p.member_id = p_member_id
        and not exists (
          select 1 from public.pet_boarding pb
          where pb.pet_id = p.id and pb.end_date >= current_date
        );
