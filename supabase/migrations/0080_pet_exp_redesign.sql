-- ============================================================
-- 0080: 宠物互动经验机制重构
-- 经验不再按血条增量发放，只有属性满值(100)才发经验
-- 体力满值+10(日1次) 清洁满值+10(日1次) 心情满值+10(日3次)
-- 单日总经验封顶50
-- 三项全满触发日产金币(日1次)
-- ============================================================
...
-- 一、创建每日经验记录表
-- ============================================================
create table if not exists public.pet_daily_exp_log (
  id uuid primary key default gen_random_uuid(),
  pet_id uuid not null references public.pets(id) on delete cascade,
  log_date date not null default current_date,
  hunger_full_count int not null default 0,
  clean_full_count int not null default 0,
  mood_full_count int not null default 0,
  daily_coin_claimed boolean not null default false,
  created_at timestamptz not null default now(),
  unique(pet_id, log_date)
);

-- 启用 RLS
alter table public.pet_daily_exp_log enable row level security;
create policy "Anyone can manage daily exp log" on public.pet_daily_exp_log
  for all using (true) with check (true);

-- ============================================================
-- 二、重写 interact_with_pet
-- ============================================================
...
-- 三、每日重置函数（凌晨调用）
-- ============================================================
-- 由于 pet_daily_exp_log 按日期分区，新的一天自动创建新记录
-- 旧记录自动过期（可通过定期清理 job 删除过期记录）
-- 属性衰减逻辑由原有定时任务处理

-- 清理30天前的记录
create or replace function public.cleanup_old_exp_logs()
returns void
language plpgsql security definer as $$
begin
  delete from public.pet_daily_exp_log where log_date < current_date - 30;
end;
$$;

grant execute on function public.cleanup_old_exp_logs() to anon, authenticated;
