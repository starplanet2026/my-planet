-- 0121: 修复萌宠星球相关 RPC 中 created_by 字段类型错误
-- 问题：coin_records.created_by 是 uuid 类型，但 0119 中多个函数把 member_id::text 写入，
--       导致购买/进化/托管/进修/小游戏等操作报错：
--       column "created_by" is of type uuid but expression is of type text
-- 修复：去掉 ::text 强制转换，直接传入 uuid 值。
-- 涉及函数：buy_boarding_card / run_daily_boarding_care / buy_pet_item /
--           buy_doghouse_upgrade / evolve_pet / claim_study_starlight / finish_game_level

-- ====== 1. buy_boarding_card ======
drop function if exists public.buy_boarding_card(uuid, uuid);
...
begin
  ...
  values (v_family_id, p_member_id, -v_cost, v_new_star,
    '购买托管卡「' || coalesce(v_item.name, '托管卡') || '」', 'purchase', 'pet_shop_item', p_item_id, p_member_id, 'star');
...
begin
  ...
  insert into public.coin_records (family_id, member_id, amount, balance_after, reason, category, ref_type, ref_id, created_by, balance_type)
  values (v_family_id, p_member_id, -v_item.price_star, v_star,
    '购买住所「' || coalesce(v_item.name, '狗屋') || '」', 'purchase', 'pet_shop_item', p_item_id, p_member_id, 'star');
...
begin
  ...
  values (v_family_id, v_pet.member_id, -v_cost, v_new_star,
    ...
    'evolve', 'pet', p_pet_id, v_pet.member_id, 'star');
...
begin
  ...
  if v_total > 0 then
    ...
    values (v_family_id, p_member_id, v_total::int, v_new_star,
      '宠物进修产出星光值', 'study', 'pet_study', null, p_member_id, 'star');
...
begin
  ...
  if v_reward > 0 then
    ...
    values (p_family_id, p_member_id, v_reward, v_star,
      ...
      'upgrade', 'game_level', p_level::text, p_member_id, 'star');
  else
