-- 0097: Clean rewrite of check_pet + interact_with_pet (ASCII only)
-- Fixes: sub_type -> type, happiness_rounds -> mood_full_count, daily_coin_earned -> daily_coin_claimed
...
-- ============================================================
-- check_pet: daily reset + per-stat decay
-- Hunger: 0-point reset or 24h after fill, max 1/day
-- Clean: 0-point reset or 24h after fill, max 1/day
...
-- ============================================================
-- interact_with_pet: use type (not sub_type), mood_full_count (not happiness_rounds)
-- ============================================================
...
begin
  ...
  -- FIX: use subcategory (type is 'supply' for all items after migration 0019)
  ...
  -- Read item's recovery_value (not hardcoded 20)
  ...
  -- Apply recovery + record fill time
  ...
  -- FIX: use mood_full_count (not happiness_rounds)
  ...
  -- FIX: use daily_coin_claimed (boolean, not daily_coin_earned)
  -- Coin: 4 stats all full -> daily coin (1x/day)
...
-- ============================================================
-- Reset ALL pets: stats to 0, clear fill times, reset decay counts
-- ============================================================
update public.pets set
  hunger = 0,
  clean = 0,
...
-- Clear today's exp logs (in case they block exp gain)
delete from public.pet_daily_exp_log where log_date = (now() at time zone 'Asia/Shanghai')::date;
